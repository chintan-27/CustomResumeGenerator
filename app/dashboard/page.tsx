            ...
            {loading ? <Skeleton className="h-64 w-full" /> : (
              <div className="bg-white rounded-2xl border border-stone-200 overflow-hidden shadow-sm">
                <div className="p-6 bg-[#0f1f18] flex items-center gap-4">
                  <div className="w-12 h-12 rounded-full bg-[#2d6a4f] flex items-center justify-center text-white font-bold text-xl flex-shrink-0">
                    {dashboardData?.user?.name?.charAt(0)?.toUpperCase() || "U"}
                  </div>
                  <div>
                    <h3 className="text-white font-bold text-lg">{dashboardData?.user?.name}</h3>
                  ...
                  <div key={i} className="bg-white rounded-2xl border border-stone-200 p-5 shadow-sm">
                    ...
                    {edu.relevant_coursework && (
                      <div className="mt-3 pt-3 border-t border-stone-100">
                        <p className="text-xs font-medium uppercase tracking-widest text-stone-400 mb-1.5" style={{ fontFamily: "var(--font-mono)" }}>
                          Coursework
                        </p>
              ...
              {dashboardData?.skills ? (
                <div className="flex flex-wrap gap-2">
                  {dashboardData.skills.split(",").map((skill: string, i: number) => (
                    <span
                      key={i}
                      className="px-3 py-1.5 bg-stone-100 border border-stone-200 rounded-full text-sm text-[#1a1a1a] font-medium hover:bg-[#2d6a4f]/10 hover:border-[#2d6a4f]/30 hover:text-[#2d6a4f] transition-colors cursor-default"
                    >
                      {skill.trim()}
                    </span>
                  ))}
